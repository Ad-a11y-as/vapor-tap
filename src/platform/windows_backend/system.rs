...
//! （[`super::process::setup_process_loopback`]）。この経路のネイティブフォーマットは
...
use super::common::{ComThread, capture_loop, init_loopback_capture, map_hr, parse_mix_format};
...
/// （フィードバック防止）。古典 loopback ではなく [`super::process::setup_process_loopback`]
...
impl WasapiSystemBackend {
    ...
    /// Shared flag that becomes true whenever the native capture worker exits.
...
impl CaptureBackend for WasapiSystemBackend {
    ...
    fn start(&mut self, sink: RawSink) -> Result<()> {
        ...
        let terminated = Arc::clone(&stop_flag);
...
///   プロセスループバック（[`super::process::setup_process_loopback`]）。`device_id` は使わない。
///
...
fn run_system_thread(
    ...
    let setup = if exclude_self {
        ...
        unsafe { super::process::setup_process_loopback(std::process::id(), ProcessMode::Exclude) }
    } else {
